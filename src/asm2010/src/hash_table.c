...
*/
static struct hash_table_entry *hash_table_create_entry(char *key, void *value,
                                                        size_t value_size);
...
int hash_table_init(struct hash_table *hash_table) {
  if (hash_table) {
    ...
    hash_table->entries = calloc(hash_sizes[hash_table->size_index],
...
int hash_table_put_sized(struct hash_table *hash_table, char *key, void *value,
                         size_t value_size) {
  size_t hash = 0;
  ...
  if (hash_table->entry_count >
          hash_sizes[hash_table->size_index] * CHT_MAX_LOAD_FACTOR &&
...
int hash_table_remove(struct hash_table *hash_table, char *key) {
  ...
  if (hash_table->entry_count <
          hash_sizes[hash_table->size_index] * CHT_MIN_LOAD_FACTOR &&
      hash_table->size_index > 0) {
    hash_table_rehash(hash_table, hash_table->size_index - 1);
  }
...
hash_table_iterator_create(struct hash_table *hash_table) {
...
hash_table_iterator_next(struct hash_table_iterator *iterator) {
  if (!iterator || !iterator->hash_table ||
      !iterator->hash_table->entry_count) {
    return 0;
  }
  ...
  while (!iterator->entry &&
         iterator->next_index < hash_sizes[iterator->hash_table->size_index]) {
...
struct hash_table_entry *hash_table_create_entry(char *key, void *value,
                                                 size_t value_size) {
  struct hash_table_entry *entry = {0};
  size_t key_length = 0;
...
static void hash_table_rehash(struct hash_table *hash_table,
                              size_t size_index) {
  size_t old_size = 0;
  size_t new_hash = 0;
  size_t i = 0;
  ...
  new_entries =
      calloc(hash_sizes[size_index], sizeof(struct hash_table_entry *));
  if (!new_entries) {
    return;
  }
