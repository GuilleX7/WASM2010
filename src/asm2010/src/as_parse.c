...
int search_line_end(char const **lineptr) {
  if (skip_spaces(lineptr) == PARSE_LINE_KEEP && **lineptr != COMMENT_MARK) {
 ...
 */
static as_parse_argument retrieve_inm(char const **lineptr, int *status,
                                      size_t max_value) {
  as_parse_argument argument =
      AS_ARGUMENT_DEFINE_OF_TYPE(AS_ARGUMENT_TYPE_INVALID);
  argument.value.inm =
      retrieve_value(lineptr, status, max_value, search_line_end);
 ...
 */
static as_parse_argument retrieve_inm_or_equ(as_parse_info *pinfo,
                                             char const **lineptr, int *status,
                                             size_t max_len, size_t max_value) {
  as_parse_argument argument =
      AS_ARGUMENT_DEFINE_OF_TYPE(AS_ARGUMENT_TYPE_INVALID);
  char *identifier =
      retrieve_alnum_identifier(lineptr, max_len, search_line_end);
...
static int search_defined_equ(as_parse_info *pinfo, char const **lineptr) {
  ...
  if (strncmp(*lineptr, EQU_KEYWORD, EQU_KEYWORD_LENGTH))
    return PARSE_LINE_KEEP;
  (*lineptr) += EQU_KEYWORD_LENGTH;

  identifier =
      retrieve_alnum_identifier(lineptr, AS_MAX_EQU_LENGTH, search_line_end);
  if (!identifier) {
    trace_log_printf(&pinfo->log,
  ...
  if (search_character(lineptr, EQU_DEFINE_MARK, search_line_end) !=
      PARSE_LINE_KEEP) {
    trace_log_printf(&pinfo->log,
  ...
  value =
      retrieve_value(lineptr, &status, CS_INS_INM_MAX_VALUE, search_line_end);
  switch (status) {
...
static unsigned char search_opcode(as_parse_info *pinfo, char const **lineptr) {
  ...
  if (strncmp(*lineptr, OPCODE_KEYWORD, OPCODE_KEYWORD_LENGTH))
    return PARSE_LINE_KEEP;
  ...
  if (search_character(lineptr, ARG_SEP_MARK, search_line_end) ==
      PARSE_LINE_END) {
    trace_log_printf(&pinfo->log,
  ...
  lsbyte = retrieve_inm_or_equ(pinfo, lineptr, &status, AS_MAX_EQU_LENGTH,
                               CS_INS_INM_MAX_VALUE);
  switch (status) {
  case RETRIEVE_VALUE_INVALID:
    trace_log_printf(
        &pinfo->log,
...
static int search_line_equ(as_parse_info *pinfo, char const **lineptr) {
  ...
  if (search_character(lineptr, EQU_LINE_MARK, search_line_end) ==
      PARSE_LINE_END) {
    trace_log_printf(&pinfo->log,
 ...
 */
static as_parse_argument search_register(char const **lineptr,
  ...
  if (**lineptr == REG_MARK) {
    (*lineptr)++;
    register_number =
        retrieve_value_decimal(lineptr, &status, CS_REGISTERS_RMAX);
  ...
  if ((indirect && **lineptr != INDIRECT_REG_END_MARK) ||
      (!indirect && **lineptr == INDIRECT_REG_END_MARK)) {
...
static int parse_instruction_fa(as_parse_info *pinfo, char const **lineptr) {
  ...
  *reg_dst = search_register(
      lineptr, (sentence->instruction->index == CS_INS_I_ST) ? true : false);
  ...
  *reg_src = search_register(
      lineptr, (sentence->instruction->index == CS_INS_I_LD) ? true : false);
...
static int parse_instruction_fb(as_parse_info *pinfo, char const **lineptr) {
  ...
  if (sentence->instruction->index == CS_INS_I_STS) {
    int status = {0};
    *inm_address = retrieve_inm_or_equ(pinfo, lineptr, &status,
...
static int parse_instruction_fc(as_parse_info *pinfo, char const **lineptr) {
  ...
  *inm_address = retrieve_inm_or_equ(pinfo, lineptr, &status, AS_MAX_EQU_LENGTH,
                                     CS_INS_INM_MAX_VALUE);
  if (inm_address->type == AS_ARGUMENT_TYPE_INVALID) {
    trace_log_printf(&pinfo->log,
...
static int search_instruction(as_parse_info *pinfo, char const **lineptr) {
  char *instruction_name = retrieve_alnum_identifier(
      lineptr, CS_INS_NAME_MAX_LENGTH, search_line_end);
  as_parse_sentence *sentence = {0};
...
int as_parse_line(as_parse_info *pinfo, char const *line) {
  ...
  if (strlen(line) > AS_MAX_LINE_LENGTH + 1) {
    trace_log_printf(&pinfo->log,
                     "[Error] invalid line length at line %" PRI_SIZET "\n",
                     pinfo->parsing_line_index);
    return AS_PARSE_ERROR;
  }
...
int as_parse_assemble(as_parse_info *pinfo, bool stop_on_error) {
  ...
  for (; i < pinfo->sentence_index; i++) {
    ...
    if (sentence->arg_a.type == AS_ARGUMENT_TYPE_EQU) {
      value_ptr =
          hash_table_get(&pinfo->equs_ht, sentence->arg_a.value.equ_key);
      if (!value_ptr) {
        trace_log_printf(
            &pinfo->log,
    ...
    if (sentence->arg_b.type == AS_ARGUMENT_TYPE_EQU) {
      value_ptr =
          hash_table_get(&pinfo->equs_ht, sentence->arg_b.value.equ_key);
      if (!value_ptr) {
        trace_log_printf(
            &pinfo->log,
            "[Error] equ '%s' of instruction '%s' at line %" PRI_SIZET
