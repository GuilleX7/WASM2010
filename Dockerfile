...
WORKDIR /sdk
RUN wget -O wasisdk.tar.gz https://github.com/WebAssembly/wasi-sdk/releases/download/wasi-sdk-16/wasi-sdk-16.0-linux.tar.gz
RUN tar -xaf wasisdk.tar.gz
RUN rm wasisdk.tar.gz
WORKDIR /app
COPY ./ ./
WORKDIR /app/src/asm2010
RUN make WASI_SDK="/sdk/wasi-sdk-16.0" all
WORKDIR /app
RUN npm ci
RUN npm run build
